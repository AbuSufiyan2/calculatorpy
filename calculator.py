...
display = tk.Entry(root, font=font_large, bd=10, insertwidth=2,
                   width=14, borderwidth=4, justify='right', bg='white')
...
for (text, row, col) in buttons:
    ...
    button = tk.Button(root, text=text, padx=20, pady=20, font=font_small,
                       command=action, bg='white', borderwidth=0)
...
buttons_dict = {
    ...
    '+': (lambda: button_click('+'), 4, 3),
    '=': (button_equal, 5, 3)
}

for btn, (cmd, r, c) in buttons_dict.items():
    button = tk.Button(root, text=btn, padx=20, pady=20, font=font_small,
                       command=cmd, bg='lightcoral', borderwidth=0)
